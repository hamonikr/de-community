		...
		<div class="card">
			...
			<div class="card-header">
				...
				<div class="card-tools">
					...
					<a href="/user/list?sort=registerDate,DESC" class="btn btn-primary purple">사용자</a>
			...
			<div class="card-body table-responsive p-0">
				<div class="row">
					<c:forEach var="list" items="${data}" varStatus="status">
						<div class="col-3">
							<a class="profileImg" href="/user/view/${list.userNo}">
								<img alt="img_${list.userName}" src="/img/HamoniKR_logo600.png">
							</a>
							<a href="/user/dashboard/${list.userNo}">${list.userName}</a>
						</div>
					</c:forEach>
			...
			<div class="card-footer row">
				...
				<c:if test="${not paging.first}">
					<a href="/user/list?page=${paging.number}">이전</a><
				</c:if>
				<c:forEach begin="1" end="${paging.totalPages}" var="idx">
					<a href="/user/list?page=${idx}">${idx}</a>
				</c:forEach>
				<c:if test="${not paging.last && paging.totalPages > 0}">
					<a href="/user/list?page=${paging.number + 2}">다음</a>
				</c:if>
			</div>
